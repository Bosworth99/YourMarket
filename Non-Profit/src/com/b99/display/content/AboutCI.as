...
//-----------------------------------------------------------------------------		
		...
		override public function animateContentIn():void
		{
			...
			TweenLite.from(_mainTitle, 		1, { delay:0.6,	x:_mainTitle.x - 50, 	y:_mainTitle.y, 		alpha:0 } );
			TweenLite.from(_mainText, 		1, { delay:0.7,	x:_mainText.x - 50, 	y:_mainText.y, 			alpha:0 } );
			TweenLite.from(_mission, 		1, { delay:1.8, x:_mission.x, 			y:_mission.y, 			alpha:0, blurFilter: { blurX:20, blurY:20, quality:2, remove:true }} );
			TweenLite.from(_btn_back, 		1, { delay:0.9,	x:_btn_back.x - 75, 	y:_btn_back.y, 			alpha:0, onComplete:animateInComplete } );
		}
...
//-----------------------------------------------------------------------------		
		...
		override public function animateContentOut():void
		{
			...
			TweenLite.to(_btn_back, 	.5, { delay:0.9,	x:_btn_back.x, 	y:_btn_back.y, 	alpha:0, onComplete:animateOutComplete } );
